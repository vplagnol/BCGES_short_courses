...
One thing before we start: please unzip the file \texttt{chr12\_short.fa.gz} in the \texttt{data/RNASeq} folder and go back to seesion 7.
...
{\Exercise:} The files below are fastq files for RNA-Seq post-mortem brain data. Can you understand why the aligned proportion is extremely low?
...
<<cutadapt, engine='bash', echo = echo, results = results>>=
