...
\clearpage
\section{Two relatively more advanced ideas}
...
java -Xmx4g -jar picard/MarkDuplicates.jar  ASSUME_SORTED=true \
      ...
      INPUT=../data/BAM_files/HG00130.mapped.ILLUMINA.bwa.GBR.exome.20130415.bam \
...
samtools view -f 0x0001 -f 0x0002 -b -o only_paired.bam GBR_1KG_exomes/HG00130.mapped.ILLUMINA.bwa.GBR.exome.20130415.bam

java -Xmx4g -jar picard/SamToFastq.jar INPUT=paired.bam FASTQ=read1.fq SECOND_END_FASTQ=read2.fq
@ 
...
samtools view paired.bam | awk '{print $1}' | sort | uniq -c | sort -r
@ 
...
samtools view -h paired.bam | awk '{if (($1 != "SRR707198.24062323") && ($1 != "SRR707198.20187117")) print}' > paired_fixed.sam
