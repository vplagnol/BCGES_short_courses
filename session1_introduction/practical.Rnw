...
pear-0.9.4-64 -e -v 10 -f ../data/fastq_for_merging/reads1.fq \
                             -r ../data/fastq_for_merging/reads2.fq \
                             -o results/merged -c 40
@

And now something very similar using flash:

<<flash, engine = 'bash', eval = FALSE>>=
flash -m 10 -o results/tough_merge ../data/fastq_for_merging/reads1.fq \
...
Have a look at the options that PEAR and FLASH use, and see how the choice of the \texttt{v} argument affects the final results.
There are some discrepancies between the output of both software. It may be interesting to understand them and assess which you think is doing the best job.
...
One good way to illustrate the importance of trimming is to follow-up on the example above of merging paired end reads.

\noindent
<<failMerge, engine ='bash', eval = FALSE>>=
...
So it looks terrible! Now we will want to fix this using trimming.
...
Do you understand where the difference comes from?

<<goodMerge, engine ='bash', eval = FALSE, results = results, echo = echo>>=
flash -m 10 -o results/tough_merge results/tough_read1_trimmed.fq\
                                   results/tough_read2_trimmed.fq \ 
                                   1> results/flash2.out 2> results/flash2.error
@ 
