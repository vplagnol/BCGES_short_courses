...
#And if FASTQ joiner does not work, it is probably because you need to run \texttt{FASTQ Groomer} first, 
#which does some magic to ``prepare'' the fastq files.  
@ 
...
/cluster/project8/vyp/vincent/Software/pear_github/PEAR-master/src/pear -e -v 10 \
